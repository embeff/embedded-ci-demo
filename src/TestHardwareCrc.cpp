#include <gtest/gtest.h>
#include <HardwareCrc.h>
#include "SoftwareCrc.h"

class TestHardwareCrc : public ::testing::Test {
protected:
    demo::HardwareCrc m_cut;

    uint8_t const SampleData[128] = {};

    void SetUp() override {

    }
};

TEST_F(TestHardwareCrc, ZeroLength_Return0) {
    EXPECT_EQ(m_cut.crc32(SampleData, 0), -1);
}

TEST_F(TestHardwareCrc, PartOfSampleData_ReturnPreCalculatedValue) {
    EXPECT_EQ(m_cut.crc32(SampleData, 16), 0x0547A4CC);
}

TEST_F(TestHardwareCrc, SampleData15Bytes_ReturnPreCalculatedValue) {
    EXPECT_EQ(m_cut.crc32(SampleData, 15), 0xa7a2ff64);
}
